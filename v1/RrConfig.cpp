...
namespace rr
{
	...
	const char* RrConfig::ReadChar(const char* section, const char* item, const char* default_value)
	{
		char* pEnd1;
		std::string tmp_s(section);
		std::string tmp_i(item);
		//const char def(default_value);
		std::map<std::string, std::string> k_v;
		std::map<std::string, std::string>::iterator it_item;
		std::map<std::string, std::map<std::string, std::string> >::iterator it;
		it = settings_.find(tmp_s);
		...
		//std::cout << "it_item->second.c_str() = " << it_item->second.c_str() << std::endl;
	...
	int RrConfig::ReadInt(const char* section, const char* item, const int& default_value)
	{
		...
		//	std::cout << "it_item->second " << atoi(it_item->second.c_str()) << std::endl;
		//	system("pause");
		//}
