...
void frame_read(vector<Mat>& frame_buffer) {
    ...
    if (reader.parse(in, root))
	{
		...
		hook_weight_double = root["hook_weight_double"].asFloat();
		...
		std::cout << "hook_weight_double =  " << hook_weight_double << std::endl;
        ...
        if (!frame_buffer.empty()) {
            ...
            if(!frame.empty()){
                ...
                if (objects.size() > 0){
                    ...
                    threshold_x = maxs(w_hook, h_hook) * hook_weight_double;
                    ...
                    if(std::abs(hook_center_point_x - standard_x) <= abnormal_detect_threadshold){
                        if(std::abs(hook_center_point_x - standard_x) > threshold_x){
                            ...
                            if(smoothing_warn_it >= smoothing_factor){
                                cv::resize(frame, send_frame, cv::Size(2560 * img_ratio, 1440 * img_ratio));
                                if(smoothing_warn_it % send_interval == smoothing_factor){
                                    level = "warn";
                                    std::cout << "level = " << level << std::endl;
                                    std::cout << "label = " << label << std::endl;
                                    tmp->send_json(send_frame, label, level);
                                }
                            }
                        }
                    }
